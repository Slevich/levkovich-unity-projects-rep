...
public class Biker : MonoBehaviour
{
    ...
    //Переменная обозначающая, падает ли враг.
    ...
    private void GenerateRandomPoint()
    {
        if (playerDetected == false)
        {
            if (wallDetected == false)
            {
                ...
                Debug.Log($"Direction number: {randomNumber}");
    ...
    /// переключая его стейт на 
    /// указанный в параметрах.
    /// </summary>
    /// <param name="enemyState"></param>
    private void StopEnemy(int enemyState)
    {
        enemyRB.velocity = Vector2.zero;
    ...
    /// <summary>
    /// Метод передает в аниматор рандомное число,
    /// переключая стейт врага на атаку (3).
    /// </summary>
    private void EnemyAttack()
    {
        if (isAttack)
        {
            enemyAnim.SetFloat("AttackNumber", Random.Range(0, 1));
            enemyStateNumber = 3;
        }
    }
    ...
    private void DebugStuff()
    {
        ...
        Debug.Log($"Enemy forward: {enemyForward}");
        Debug.Log($"Enemy backward: {enemyBackward}");
